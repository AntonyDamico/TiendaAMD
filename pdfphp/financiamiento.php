...
function Header()
{
	...
	$this->Ln(10);
	$this->Cell(225);
	$this->Cell(70,10,'Fecha : '.$fecha,0,1,'C');
	$this->SetFont('Arial','B',15);
	$this->Ln(5);
	//$this->Cell(110);
	$nestatus=$_SESSION['nestatus'];
	$this->Cell(275,10,'SOLICITUD DE FINANCIAMIENTO',0,1,'C');
...
include('../conex.php');
			 ...
			 // $wsql="SELECT * FROM financiamiento inner join productor on financiamiento.idproductor=productor.idproductor inner join scartaproductor on scartaproductor.idproductor=productor.idproductor";
			 $wsql= $_SESSION['wsql'];
			...
			$pdf->Cell(18,5,number_format($monto,0,',','.'),1,1,'R');
