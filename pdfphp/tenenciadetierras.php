...
function Header()
{
	...
	$this->Cell(275,10,'SOLICITUD DE TENENCIA DE TIERRAS',0,1,'C');
	...
	$this->Cell(60,10,'SECTOR',1,'C');
    $this->Cell(60,10,'PRODUCTOR',1,0,'C');
...
include('../conex.php');
			...
			$pdf->Cell(60,5,utf8_encode($row2['sector']),1,0,'L');
			$pdf->Cell(60,5,utf8_encode(strtoupper($row['nombre'])."  ".strtoupper($row['apellido'])),1,0,'L');
			$pdf->Cell(20,5,$row['cedula'],1,0,'C');
			$pdf->Cell(33,5,utf8_encode(strtoupper($row5['nombre'])),1,1,'L');

	
	
			  }
			  $pdf->Cell(273,5,'Total Productores : '.$i,1,1,'C');
$pdf->Output();
