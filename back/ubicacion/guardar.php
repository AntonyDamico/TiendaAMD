...
//variables locales
	...
	$wsqli="select * from ubicacion where nombre = '$nombre' "; //la variable se llama cursor (WorkingSQLI)
	$result = $linki->query($wsqli);
	if($linki->errno) die($linki->error);//Mostrar error si hay alguno
	$row = $result->fetch_array();
	if($row > 0){
		$_SESSION['tm']=0; //Variable de sesion
		$_SESSION['mensaje']="La ubicacion <b>".$nombre."</b> ya existe.";
	} else {
		...
		$_SESSION['mensaje']="La ubicacion <b>".$nombre."</b> se registró con exito.";
	}
